...
# -a : Prints the result on the libnotify/notify-osd notification service
...
_ALERT_IMG_LOC_="$(pwd)/img/ratp.png"
...
if [ $# = 1 ]
then
    DISPLAY=:0.0 XAUTHORITY=~/.Xauthority notify-send -u critical -i "${_ALERT_IMG_LOC_}" "Next RATP Bus Arrivals" "Route: B${RATP_BUS_NO}\nDirection: ${RATP_DIRECTION}\nStop: ${RATP_STOP_NAME}\nArriving ${TIME1} min and ${TIME2} min"
else
